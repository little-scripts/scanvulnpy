...
class TestGetRequirements:
    ...
    def test_get_requirements_with_path_and_no_freeze(self, scanner, tmp_path):
        ...
        packages, nb_packages = scanner.get_requirements(str(requirements_file))

        # Check that the method returns the correct packages
        assert packages == ["package1", "package2", "package3"]
        assert nb_packages == 3

    def test_get_requirements_without_path_and_freeze(self, scanner, monkeypatch):
        ...
        packages, nb_packages = scanner.get_requirements(path=False)

        # Check that the method returns the correct packages
        assert packages == ["packageA", "packageB", "packageC"]
        assert nb_packages == 3

    def test_get_requirements_without_path_and_no_freeze(self, scanner, monkeypatch):
        ...
        packages, nb_packages = scanner.get_requirements(path=None)
